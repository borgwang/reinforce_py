...
class DDPG(object):
    def __init__(self, env):
        ...
        self.ou = OUNoise(self.action_dim, theta=self.noise_theta, sigma=self.noise_sigma)
    ...
    def construct_model(self, gpu):
        ...
        with tf.device(device):
            # output action and q_value and compute gradients of q_val w.r.t. action
            ...
            with tf.name_scope('update_target_networks'):
                ...
                for v_source, v_target in zip(self.actor_params, self.target_actor_params):
                    update_op = v_target.assign_sub(
                            self.target_update_rate * (v_target - v_source))
                    target_networks_update.append(update_op)

                for v_source, v_target in zip(self.critic_params, self.target_critic_params):
                    update_op = v_target.assign_sub(
                            self.target_update_rate * (v_target - v_source))
                    target_networks_update.append(update_op)
    ...
    def actor(self, states, bn=False):
        ...
        if bn:
            states = self.batch_norm(states, self.is_training, tf.identity, scope='actor_bn_states')

        w1 = tf.get_variable("w1", [self.state_dim, self.h1_dim], initializer=init)
        b1 = tf.get_variable("b1", [self.h1_dim], initializer=init)
        h1 = tf.matmul(states, w1) + b1
        if bn:
            h1 = self.batch_norm(h1, self.is_training, tf.nn.relu, scope='actor_bn_h1')

        w2 = tf.get_variable("w2", [self.h1_dim, self.h2_dim], initializer=init)
        b2 = tf.get_variable("b2", [self.h2_dim], initializer=init)
        h2 = tf.matmul(h1, w2) + b2
        if bn:
            h2 = self.batch_norm(h2, self.is_training, tf.nn.relu, scope='actor_bn_h2')

        # use tanh to bound the action
        w3 = tf.get_variable("w3", [self.h2_dim, self.action_dim],
    ...
    def critic(self, states, action, bn=False):
        init = tf.contrib.layers.variance_scaling_initializer(
                factor=1.0, mode='FAN_IN', uniform=True)
        if bn:
            states = self.batch_norm(states, self.is_training, tf.identity, scope='critic_bn_states')

        w1 = tf.get_variable("w1", [self.state_dim, self.h1_dim], initializer=init)
        b1 = tf.get_variable("b1", [self.h1_dim], initializer=init)
        h1 = tf.matmul(states, w1) + b1
        if bn:
            h1 = self.batch_norm(h1, self.is_training, tf.nn.relu, scope='critic_bn_h1')

        # skip action from the first layer
        h1_concat = tf.concat([h1, action], 1)

        w2 = tf.get_variable("w2", [self.h1_dim+self.action_dim, self.h2_dim], initializer=init)
        b2 = tf.get_variable("b2", [self.h2_dim], initializer=init)
        h2 = tf.nn.relu(tf.matmul(h1_concat, w2) + b2)

        w3 = tf.get_variable("w3", [self.h2_dim, 1],
                           initializer=tf.random_uniform_initializer(-3e-3, 3e-3))
