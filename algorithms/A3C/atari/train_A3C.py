...
def main(args):
    ...
    for i in range(num_workers):
        ...
        w = Worker(i, Atari(args),
                global_steps_counter, worker_summary_writer,  args)
        workers.append(w)
    ...
    with tf.Session() as sess:
        ...
        if args.eval_every > 0:
            evaluator = Evaluate(global_net, summary_writer, global_steps_counter, args)
            evaluate_thread = threading.Thread(target=lambda: evaluator.run(sess, coord))
...
def args_parse():
    ...
    parser.add_argument('--save_path', default='/tmp/a3c', type=str,
    ...
    parser.add_argument('--save_videos', default=True, type=bool,
            help='Whether to save videos when evaluating')
    # hyperparameters
    parser.add_argument('--init_learning_rate', default=7e-4, type=float,
            help='Learning rate of the optimizer')
    parser.add_argument('--decay', default=0.99, type=float,
            help='decay factor of the RMSProp optimizer')
    parser.add_argument('--smooth', default=1e-7, type=float,
    ...
    parser.add_argument('--entropy_ratio', default=0.01, type=float,
            help='Initial weight of entropy loss')
    parser.add_argument('--clip_grads', default=40.0, type=float,
